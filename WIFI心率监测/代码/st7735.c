...
void PutPixel(uint x_start,uint y_start,uint color)//画一个像素点
{
	Lcd_SetRegion(x_start,y_start,x_start+1,y_start+1);
	LCD_WriteData_16Bit(color);
	
}
void put_bigpixel(uint x,uint y,uint color) //画一个大小4个像素点的店
{
	PutPixel(x,y,color);
...
void Display_ASCII8X16(uint x0,uint y0,uchar *s) //显示一个字符
{
	int i,j,k,x,y,xx;
